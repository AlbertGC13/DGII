...
import { isEcf31ItemDescriptionMetadataEvidence } from "../domain/ecf31-item-description-metadata-evidence.js";
...
import { isEcf31SubquantityMetadataEvidence } from "../domain/ecf31-subquantity-metadata-evidence.js";
...
import { isEcf31OtherCurrencyDetailEvidence } from "../domain/ecf31-other-currency-detail-evidence.js";
import type { Ecf31OtherCurrencyDetailEvidence } from "../domain/ecf31-other-currency-detail-evidence.js";
...
import type { Ecf31LineSubadjustmentEvidence } from "../domain/ecf31-line-subadjustment-evidence.js";
...
export type Ecf31DetallesItemsXmlMapperErrorCode =
  ...
  | "INVALID_ECF31_DETALLES_ITEMS_XML_LINE_SUBADJUSTMENT_EVIDENCE"
  | "ECF31_DETALLES_ITEMS_XML_LINE_SUBADJUSTMENT_LINEAGE_MISMATCH"
  ...
  | "INVALID_ECF31_DETALLES_ITEMS_XML_SUBQUANTITY_METADATA"
  ...
  | "INVALID_ECF31_DETALLES_ITEMS_XML_OTHER_CURRENCY_DETAIL_EVIDENCE"
  | "ECF31_DETALLES_ITEMS_XML_OTHER_CURRENCY_DETAIL_LINEAGE_MISMATCH"
...
type Input = Readonly<{
  evidence: unknown;
  lineSubadjustmentEvidence?: unknown;
  ...
  otherCurrencyDetailEvidence?: unknown;
}>;
...
const MESSAGES: Readonly<Record<Ecf31DetallesItemsXmlMapperErrorCode, string>> = Object.freeze({
  ...
  INVALID_ECF31_DETALLES_ITEMS_XML_LINE_SUBADJUSTMENT_EVIDENCE: "e-CF 31 DetallesItems XML mapper requires genuine line subadjustment evidence.",
  ECF31_DETALLES_ITEMS_XML_LINE_SUBADJUSTMENT_LINEAGE_MISMATCH: "e-CF 31 DetallesItems XML mapper line subadjustment evidence lineage does not match its evidence.",
  ...
  INVALID_ECF31_DETALLES_ITEMS_XML_SUBQUANTITY_METADATA: "e-CF 31 DetallesItems XML mapper requires genuine subquantity metadata evidence.",
  ...
  INVALID_ECF31_DETALLES_ITEMS_XML_OTHER_CURRENCY_DETAIL_EVIDENCE: "e-CF 31 DetallesItems XML mapper requires genuine other-currency detail evidence.",
  ECF31_DETALLES_ITEMS_XML_OTHER_CURRENCY_DETAIL_LINEAGE_MISMATCH: "e-CF 31 DetallesItems XML mapper other-currency detail evidence lineage does not match its evidence.",
...
function readInput(input: unknown): Input | undefined {
  try {
    ...
    if (keys.length < 1 || keys.length > 11 || !keys.every((key) => key === "evidence" || key === "lineSubadjustmentEvidence" || key === "itemCodeMetadataEvidence" || key === "descriptionMetadataEvidence" || key === "itemUnitMetadataEvidence" || key === "itemReferenceMetadataEvidence" || key === "subquantityMetadataEvidence" || key === "alcoholReferencePriceEvidence" || key === "itemDatesMetadataEvidence" || key === "retentionMetadataEvidence" || key === "otherCurrencyDetailEvidence")) return undefined;
    ...
    const otherCurrencyDetailEvidence = Object.getOwnPropertyDescriptor(input, "otherCurrencyDetailEvidence");
    ...
    if (descriptionMetadataEvidence !== undefined
    ...
    if (subquantityMetadataEvidence !== undefined
      && (!("value" in subquantityMetadataEvidence) || !subquantityMetadataEvidence.enumerable || subquantityMetadataEvidence.value === undefined)) return undefined;
    if (alcoholReferencePriceEvidence !== undefined
    ...
    if (otherCurrencyDetailEvidence !== undefined
      && (!("value" in otherCurrencyDetailEvidence) || !otherCurrencyDetailEvidence.enumerable || otherCurrencyDetailEvidence.value === undefined)) return undefined;
    return Object.freeze({
      ...
      otherCurrencyDetailEvidence: otherCurrencyDetailEvidence?.value as unknown,
    });
  } catch {
    return undefined;
  }
...
function hasMatchingLineSubadjustmentEvidence(
  subadjustmentEvidence: Ecf31LineSubadjustmentEvidence,
  evidence: Ecf31DetallesItemsEvidence,
): boolean {
  return subadjustmentEvidence.draft === evidence.draft
    && subadjustmentEvidence.entries.length === evidence.entries.length
    && subadjustmentEvidence.entries.every((entry, index) => entry.source === evidence.draft.lineAmounts[index]
      && entry.source === evidence.entries[index]?.lineAmount);
}

function subadjustmentTableElement(
  tableName: string,
  entryName: string,
  typeName: string,
  percentageName: string,
  amountName: string,
  entries: readonly Ecf31LineSubadjustmentEvidence["entries"][number]["discounts"][number][],
): XmlElement | undefined {
  if (entries.length === 0) return undefined;
  const children = entries.map((entry) => {
    const subadjustment = createXmlParentElement(entryName, [
      textElement(typeName, entry.type),
...
function hasMatchingDescriptionMetadata(
  metadata: Ecf31ItemDescriptionMetadataEvidence,
  evidence: Ecf31DetallesItemsEvidence,
): boolean {
  return metadata.draft === evidence.draft
    && metadata.entries.length === evidence.entries.length
...
function hasMatchingItemUnitMetadata(
  metadata: Ecf31ItemUnitMetadataEvidence,
  evidence: Ecf31DetallesItemsEvidence,
): boolean {
  return metadata.draft === evidence.draft
    && metadata.entries.length === evidence.entries.length
...
function hasMatchingSubquantityMetadata(
  metadata: Ecf31SubquantityMetadataEvidence,
  evidence: Ecf31DetallesItemsEvidence,
): boolean {
  return metadata.draft === evidence.draft
    && metadata.entries.length === evidence.entries.length
...
function hasMatchingAlcoholReferencePriceMetadata(
  metadata: Ecf31AlcoholReferencePriceEvidence,
  evidence: Ecf31DetallesItemsEvidence,
): boolean {
  return metadata.draft === evidence.draft
    && metadata.entries.length === evidence.entries.length
...
function hasMatchingOtherCurrencyDetail(
  metadata: Ecf31OtherCurrencyDetailEvidence,
  evidence: Ecf31DetallesItemsEvidence,
): boolean {
  return metadata.draft === evidence.draft
    && metadata.entries.length === evidence.entries.length
...
function otherCurrencyDetailElement(
  metadata: Ecf31OtherCurrencyDetailEvidence | undefined,
  index: number,
): XmlElement | undefined {
  const detail = metadata?.entries[index];
  if (detail === undefined || (detail.precioOtraMoneda === undefined && detail.descuento === undefined
    && detail.recargo === undefined && detail.montoItemOtraMoneda === undefined)) return undefined;
  const result = createXmlParentElement("OtraMonedaDetalle", [
    ...(detail.precioOtraMoneda === undefined ? [] : [textElement("PrecioOtraMoneda", formatDecimal(detail.precioOtraMoneda))]),
    ...(detail.descuento === undefined ? [] : [textElement("DescuentoOtraMoneda", formatDecimal(detail.descuento))]),
    ...(detail.recargo === undefined ? [] : [textElement("RecargoOtraMoneda", formatDecimal(detail.recargo))]),
    ...(detail.montoItemOtraMoneda === undefined ? [] : [textElement("MontoItemOtraMoneda", formatDecimal(detail.montoItemOtraMoneda))]),
  ]);
  /* v8 ignore next -- authenticated values and fixed child names form a valid detail element. */
  if (!result.ok) throw new Error("XML mapping failed.");
  return result.value;
}
...
function subquantityTableElement(metadata: Ecf31SubquantityMetadataEvidence, index: number): XmlElement | undefined {
  ...
  const children = pairs.map((pair) => {
    const item = createXmlParentElement("SubcantidadItem", [
      textElement("Subcantidad", formatDecimal(pair.subquantity)),
      textElement("CodigoSubcantidad", formatEcf31UnitOfMeasureCode(pair.unit)),
    ]);
    /* v8 ignore next -- authenticated complete pairs and fixed names form valid child elements. */
    if (!item.ok) throw new Error("XML mapping failed.");
    return item.value;
  });
  const table = createXmlParentElement("TablaSubcantidad", children);
  /* v8 ignore next -- authenticated one-to-five child elements form a valid table. */
  if (!table.ok) throw new Error("XML mapping failed.");
  return table.value;
}
...
function itemElement(
  ...
  otherCurrencyDetailEvidence: Ecf31OtherCurrencyDetailEvidence | undefined,
  ...
  const otherCurrencyDetail = otherCurrencyDetailElement(otherCurrencyDetailEvidence, index);
  ...
  const item = createXmlParentElement("Item", [
    ...
    ...(subquantityTable === undefined ? [] : [subquantityTable]),
    ...(alcoholEntry?.alcoholDegrees === undefined ? [] : [textElement("GradosAlcohol", formatDecimal(alcoholEntry.alcoholDegrees))]),
    ...(alcoholEntry?.referenceUnitPrice === undefined ? [] : [textElement("PrecioUnitarioReferencia", formatDecimal(alcoholEntry.referenceUnitPrice))]),
    ...
    ...(otherCurrencyDetail === undefined ? [] : [otherCurrencyDetail]),
...
function unsupportedFeature(
  evidence: Ecf31DetallesItemsEvidence,
  lineSubadjustmentEvidence: Ecf31LineSubadjustmentEvidence | undefined,
): Ecf31DetallesItemsXmlMapperErrorCode | undefined {
  for (const [index, entry] of evidence.entries.entries()) {
    if (formatDecimal(entry.lineAmount.discountAmount) !== "0"
...
export function mapEcf31DetallesItemsXmlElement(input: unknown): Result<XmlElement, Ecf31DetallesItemsXmlMapperError> {
  ...
  if (candidate.lineSubadjustmentEvidence !== undefined
    && !isEcf31LineSubadjustmentEvidence(candidate.lineSubadjustmentEvidence)) {
    return failure("INVALID_ECF31_DETALLES_ITEMS_XML_LINE_SUBADJUSTMENT_EVIDENCE");
  }
  const lineSubadjustmentEvidence = candidate.lineSubadjustmentEvidence;
  if (lineSubadjustmentEvidence !== undefined
    && !hasMatchingLineSubadjustmentEvidence(lineSubadjustmentEvidence, evidence)) {
    return failure("ECF31_DETALLES_ITEMS_XML_LINE_SUBADJUSTMENT_LINEAGE_MISMATCH");
  }
  ...
  if (candidate.descriptionMetadataEvidence !== undefined
    && !isEcf31ItemDescriptionMetadataEvidence(candidate.descriptionMetadataEvidence)) {
    return failure("INVALID_ECF31_DETALLES_ITEMS_XML_DESCRIPTION_METADATA");
  }
  const descriptionMetadataEvidence = candidate.descriptionMetadataEvidence;
  if (descriptionMetadataEvidence !== undefined && !hasMatchingDescriptionMetadata(descriptionMetadataEvidence, evidence)) {
    return failure("ECF31_DETALLES_ITEMS_XML_DESCRIPTION_METADATA_LINEAGE_MISMATCH");
  }
  if (candidate.itemUnitMetadataEvidence !== undefined
    && !isEcf31ItemUnitMetadataEvidence(candidate.itemUnitMetadataEvidence)) {
    return failure("INVALID_ECF31_DETALLES_ITEMS_XML_ITEM_UNIT_METADATA");
  }
  const itemUnitMetadataEvidence = candidate.itemUnitMetadataEvidence;
  ...
  if (candidate.subquantityMetadataEvidence !== undefined
    && !isEcf31SubquantityMetadataEvidence(candidate.subquantityMetadataEvidence)) {
    return failure("INVALID_ECF31_DETALLES_ITEMS_XML_SUBQUANTITY_METADATA");
  }
  const subquantityMetadataEvidence = candidate.subquantityMetadataEvidence;
  if (subquantityMetadataEvidence !== undefined && !hasMatchingSubquantityMetadata(subquantityMetadataEvidence, evidence)) {
    return failure("ECF31_DETALLES_ITEMS_XML_SUBQUANTITY_METADATA_LINEAGE_MISMATCH");
  }
  if (candidate.alcoholReferencePriceEvidence !== undefined
    && !isEcf31AlcoholReferencePriceEvidence(candidate.alcoholReferencePriceEvidence)) {
    return failure("INVALID_ECF31_DETALLES_ITEMS_XML_ALCOHOL_REFERENCE_PRICE_METADATA");
  }
  const alcoholReferencePriceEvidence = candidate.alcoholReferencePriceEvidence;
  if (alcoholReferencePriceEvidence !== undefined && !hasMatchingAlcoholReferencePriceMetadata(alcoholReferencePriceEvidence, evidence)) {
    return failure("ECF31_DETALLES_ITEMS_XML_ALCOHOL_REFERENCE_PRICE_METADATA_LINEAGE_MISMATCH");
  }
  ...
  const itemDatesMetadataEvidence = candidate.itemDatesMetadataEvidence;
  ...
  if (candidate.otherCurrencyDetailEvidence !== undefined
    && !isEcf31OtherCurrencyDetailEvidence(candidate.otherCurrencyDetailEvidence)) {
    return failure("INVALID_ECF31_DETALLES_ITEMS_XML_OTHER_CURRENCY_DETAIL_EVIDENCE");
  }
  const otherCurrencyDetailEvidence = candidate.otherCurrencyDetailEvidence;
  if (otherCurrencyDetailEvidence !== undefined && !hasMatchingOtherCurrencyDetail(otherCurrencyDetailEvidence, evidence)) {
    return failure("ECF31_DETALLES_ITEMS_XML_OTHER_CURRENCY_DETAIL_LINEAGE_MISMATCH");
  }
