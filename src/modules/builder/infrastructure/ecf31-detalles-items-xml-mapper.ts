...
function readInput(input: unknown): Input | undefined {
  try {
    ...
    if (keys.length < 1 || keys.length > 8 || !keys.every((key) => key === "evidence" || key === "lineSubadjustmentEvidence" || key === "itemCodeMetadataEvidence" || key === "descriptionMetadataEvidence" || key === "itemUnitMetadataEvidence" || key === "itemReferenceMetadataEvidence" || key === "subquantityMetadataEvidence" || key === "itemDatesMetadataEvidence")) return undefined;
...
export function mapEcf31DetallesItemsXmlElement(input: unknown): Result<XmlElement, Ecf31DetallesItemsXmlMapperError> {
  ...
  try {
    const items = evidence.entries.map((_, index) => itemElement(evidence, itemCodeMetadataEvidence, descriptionMetadataEvidence, itemUnitMetadataEvidence, itemReferenceMetadataEvidence, subquantityMetadataEvidence, itemDatesMetadataEvidence, lineSubadjustmentEvidence, index));
