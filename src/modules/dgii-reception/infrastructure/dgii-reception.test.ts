...
function reception(post: (input: unknown) => Promise<unknown>, authorization: unknown = Object.freeze(Object.create(null))) {
...
describe("DGII reception", () => {
  it("submits only a genuine verified artifact as the official XML file and returns a TrackId", async () => {
    ...
    const result = await reception((input) => { request = input; return Promise.resolve({ status: 200, mediaType: "application/json", body: '{"trackId":"track-142","error":null,"mensaje":null}' }); }).submit(source.artifact);
  ...
  it("rejects forged artifacts, malformed authenticated identities, hostile capabilities, and contained failures", async () => {
    ...
    for (const configuration of [null, {}, { authentication: 1 }, { authentication: { authorize: 1, postMultipart() {} } }, { authentication: { authorize() {}, postMultipart: 1 } }, new Proxy({}, { getPrototypeOf() { throw new Error("secret"); } }), new Proxy({}, { ownKeys() { throw new Error("secret"); } })]) {
  ...
  it("requires exact success JSON fields and rejects substantive errors without admitting polling", async () => {
    ...
    ]) await expect(reception(() => Promise.resolve(response)).submit(source)).resolves.toEqual({ ok: false, error: { code: "DGII_RECEPTION_FAILED" } });

    await expect(reception(() => Promise.resolve({ status: 200, mediaType: "application/json", body: '{"trackId":" x ","error":" ","mensaje":" "}' })).submit(source)).resolves.toEqual({ ok: true, value: { trackId: "x" } });
    const unavailable = api.createDgiiReception({ authentication: { authorize: () => Promise.resolve({ ok: false, error: { code: "DGII_AUTHENTICATION_FAILED" } }), postMultipart: () => Promise.resolve({}) } });
  ...
  it("requires one direct identity hierarchy and bounds hostile response fields before parsing", async () => {
    ...
    await expect(reception(() => Promise.resolve({ status: 200, mediaType: "application/json", body: '{"trackId":"x","error":null,"mensaje":null}' })).submit((await artifact(nested)).artifact)).resolves.toMatchObject({ ok: false });
    ...
    for (const body of ["x".repeat(65_537), JSON.stringify({ trackId: "x".repeat(257), error: null, mensaje: null }), JSON.stringify({ trackId: "x", error: "x".repeat(257), mensaje: null }), JSON.stringify({ trackId: "x", error: null, mensaje: "x".repeat(257) })]) {
      await expect(reception(() => Promise.resolve({ status: 200, mediaType: "application/json", body })).submit(valid)).resolves.toMatchObject({ ok: false });
    }
  });
  ...
  it("acquires opaque authorization before its owning capability applies it", async () => {
    ...
    const result = api.createDgiiReception({ authentication: { authorize: () => Promise.resolve({ ok: true, value: owner }), postMultipart: (authorization: unknown) => { received = authorization; return Promise.resolve({ status: 200, mediaType: "application/json", body: '{"trackId":"x","error":null,"mensaje":null}' }); } } });
    if (!result.ok) throw new Error("Expected reception.");
