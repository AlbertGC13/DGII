...
const loaded = loadInMemoryPkcs12({ bytes: Buffer.from(forge.asn1.toDer(p12).getBytes(), "binary"), password, expectedSignerIdentity: identity.value });
...
signature.addReference({ xpath: "/*", transforms: [enveloped, c14n], digestAlgorithm: sha256, isEmptyUri: true });
// The fixture mirrors the DGII semilla: several namespace declarations in non-canonical order, so a
// digest taken over a plain serialization instead of the canonical octet stream fails this smoke.
const source = '<SemillaModel xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema"><valor>synthetic</valor><fecha>2026-08-19T22:54:46.7869709-04:00</fecha></SemillaModel>';
signature.computeSignature(source, { location: { reference: "/*", action: "append" } });
const signedXml = signature.getSignedXml();

for (const expected of [`<CanonicalizationMethod Algorithm="${c14n}"/>`, `<SignatureMethod Algorithm="${rsaSha256}"/>`, '<Reference URI="">', `<Transforms><Transform Algorithm="${enveloped}"/><Transform Algorithm="${c14n}"/></Transforms>`, `<DigestMethod Algorithm="${sha256}"/>`, "<KeyInfo><X509Data><X509Certificate>"]) {
  if (!signedXml.includes(expected)) throw new Error(`Expected XMLDSig profile fragment is absent: ${expected}`);
}
if (!signedXml.endsWith("</Signature></SemillaModel>")) throw new Error("Signature is not the final root child.");
